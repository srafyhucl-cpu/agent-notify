...
test.describe("交互一致性（整块可点）", () => {
  test("Agent 卡片：头部整块可点、chevron 可点、开关不误触折叠、键盘仍可用", async ({
    ...
    // 默认展开首张卡片（与渠道页默认选中首个账号一致）；披露语义用 aria-expanded
    await expect(titleButton).toHaveAttribute("aria-expanded", "true");
    await expect(titleButton).toHaveAttribute("aria-controls", "agent-body-alpha");
    ...
    // 收起后卡片正文被卸载：不再写 aria-controls，避免指向不存在的 ID
    await expect(titleButton).not.toHaveAttribute("aria-controls");
  ...
  test("历史行：整行可点、选中态与渠道页同款（左侧主色药丸）、键盘仍可用", async ({
    ...
    const firstTitle = firstRow.getByRole("button", { name: "构建完成" });
    const secondTitle = rows.nth(1).getByRole("button", { name: "投递失败" });
    ...
    // 详情面板始终在（无选中时是空态），所以 aria-controls 常驻
    await expect(firstTitle).toHaveAttribute("aria-controls", "history-detail");
    ...
    await page.keyboard.press("Enter");
    await expect(rows.nth(1)).toHaveClass(/history-row--selected/);
    await expect(firstRow).not.toHaveClass(/history-row--selected/);
    await expect(secondTitle).toHaveAttribute("aria-expanded", "true");
    await expect(firstTitle).toHaveAttribute("aria-expanded", "false");
  });
});
