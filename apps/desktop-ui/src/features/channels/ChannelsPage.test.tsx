...
describe("ChannelsPage", () => {
  ...
  it("surfaces a skipped test delivery instead of claiming success", async () => {
    const user = userEvent.setup();
    ...
    const bridge = createMockHostBridge({
      ...
      sendTestDelivery: {
        ...
        error: {
          ...
          message: "ClawBot 主动推送会话已失效，请先给 ClawBot 发送一条消息",
        },
        retryable: true,
    ...
    // 被跳过（如主动推送会话失效）必须如实报错（错误长期可见），不能弹"已发送"浮层
