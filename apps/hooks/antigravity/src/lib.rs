//! Antigravity Stop Hook 的纯逻辑：读取有界 stdin，把原始 Stop 事件按 `protocolVersion=1`
//! 的 `agent.event` 提交给 `agentnotify-ingress.exe`，并且**始终**输出 Antigravity 要求的 `{}`。
//!
//! 顺序契约：Hook 只提交事件与写诊断，不改变 Antigravity 的退出语义；
...
const SELECTOR_ARGUMENT: &str = "antigravity";
/// 只有被当作 Stop Hook 调用（`antigravity stop`）时才提交事件。
...
/// 诊断日志沿用 Go 版目录习惯：`%TEMP%\agent-notify\antigravity-notify-debug.log`。
const DEBUG_DIR_NAME: &str = "agent-notify";
const DEBUG_FILE_NAME: &str = "antigravity-notify-debug.log";
...
impl IngressFailure {
    fn message(&self) -> String {
        match self {
            Self::NotFound => format!("未找到 {INGRESS_EXE_NAME}，Antigravity 事件未提交"),
            ...
            Self::Exit {
                ...
                "ingress 退出码 {PROTOCOL_REJECTED_EXIT_CODE}：事件被协议拒绝（正文或 payload 超限），Antigravity 事件未提交"
            ),
            Self::Exit { code: Some(code) } => {
                format!("ingress 退出码 {code}，Antigravity 事件未提交")
            }
            Self::Exit { code: None } => "ingress 异常退出，Antigravity 事件未提交".to_owned(),
            Self::Timeout => "ingress 提交超时，Antigravity 事件未提交".to_owned(),
            Self::Envelope(error) => format!("无法编码 Antigravity 事件：{error}"),
        }
    }
}
...
/// 只在 `antigravity stop` 调用下提交事件；其余调用什么都不做。
...
/// 消费首个 `antigravity` 选择参数后，首个参数必须是 `stop`。
...
pub fn build_envelope(stdin: &[u8]) -> Result<Vec<u8>, String> {
    ...
    serde_json::to_vec(&envelope).map_err(|error| format!("无法编码 Antigravity 事件：{error}"))
}
...
/// 诊断日志路径：`AGENT_NOTIFY_TEMP_DIR` 优先，其次 `%TEMP%\agent-notify\antigravity-notify-debug.log`。
...
mod tests {
    ...
    async fn stop_invocation_submits_exactly_one_event() {
        ...
        let outcome = run_hook(
            &stop_args(),
            br#"{"conversationId":"conversation-1","fullyIdle":true}"#,
            &programs,
        )
        .await;
        ...
        assert_eq!(envelope["payload"]["conversationId"], "conversation-1");
    ...
    async fn non_stop_invocation_never_touches_ingress() {
        ...
        let outcome = run_hook(&args(&["antigravity", "status"]), b"{}", &programs).await;
    ...
    fn only_stop_invocations_are_handled() {
        assert!(is_stop_invocation(&args(&["antigravity", "stop"])));
        assert!(is_stop_invocation(&args(&["stop"])));
        assert!(is_stop_invocation(&args(&["ANTIGRAVITY", "STOP"])));
        assert!(!is_stop_invocation(&args(&[])));
        assert!(!is_stop_invocation(&args(&["antigravity", "status"])));
        assert!(!is_stop_invocation(&args(&["codex", "stop"])));
    }

    #[test]
    fn envelope_carries_the_raw_stop_payload() {
        let stdin = br#"{"conversationId":"conversation-1","fullyIdle":true,"transcriptPath":"D:\\t.jsonl"}"#;
        ...
        assert_eq!(envelope["payload"]["conversationId"], "conversation-1");
        assert_eq!(envelope["payload"]["fullyIdle"], true);
    }
    ...
    fn stdin_without_json_or_with_bom_falls_back_to_an_empty_payload() {
        ...
        let bytes = build_envelope("\u{feff}{\"fullyIdle\":true}".as_bytes()).unwrap();
        let envelope: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(envelope["payload"]["fullyIdle"], true);
    }
    ...
    fn debug_log_is_bounded_and_contains_reason() {
        ...
        let path = directory
            .path()
            .join("agent-notify")
            .join("antigravity-notify-debug.log");
    ...
    fn debug_log_write_failure_is_reported_to_caller() {
        ...
        let result = append_debug_line_to(&blocker.join("antigravity-notify-debug.log"), "x");
