...
// testNotificationBody 返回自检通知正文。
// 自检通知的 Agent 是 "test"、不携带会话 ID，isRouteable 因此为 false，
// 不会写入引用路由；文案里不能声称可以引用续聊。
func testNotificationBody() string {
	return "| 项目 | 状态 | 详情 |\n" +
		"|---|---|---|\n" +
		"| ClawBot 登录 | 正常 | 凭据有效 |\n" +
		"| 会话状态 | 就绪 | 主动推送已联通 |\n" +
		"| 桌面客户端 | 运行中 | v" + app.Version + " |\n\n" +
		"> 提示：本条为链路自检，不能用于引用续聊；引用续聊请在真实 Agent 通知上验证。"
}
