...
function readJob(pathname) {
  ...
  if (!SESSION_ID_PATTERN.test(parsed.sessionID.trim())) {
    throw new Error("invalid reply session id")
  }
  if (parsed.text.length > MAX_REPLY_TEXT_CHARS) {
    throw new Error("invalid reply text")
  }
  // targetID 是桌面端内部 Cascade 标识；缺省表示与本地会话号相同（旧版任务）。
  let targetID = ""
  if (parsed.targetID !== undefined) {
    if (typeof parsed.targetID !== "string" || !SESSION_ID_PATTERN.test(parsed.targetID.trim())) {
      throw new Error("invalid reply target id")
    }
    targetID = parsed.targetID.trim()
  }
  return { ...parsed, sessionID: parsed.sessionID.trim(), targetID }
}
...
function createSpoolStore(directory, log) {
  ...
  function claim(jobID) {
    ...
    try {
      ...
      if (job.id !== jobID) {
        throw new Error("reply job id does not match filename")
      }
    } catch (error) {
      writeResult(
        jobID,
...
function createCascadeBridge({
  ...
  async function send(job) {
    ...
    const targetID = job.targetID || job.sessionID
    ...
    try {
      ...
      for (const jobID of store.listPendingJobIDs()) {
        ...
        try {
          ...
          log(
            `reply failed session=${claimed.job.sessionID} cascade=${claimed.job.targetID || claimed.job.sessionID} code=${code} detail=${detail}${causeSuffix}`,
          )
