function Get-LinkWeixinPaths {
  ...
  .DESCRIPTION
    ...
    - AntigravityMarker：ANTIGRAVITY_NOTIFY_MARKER_FILE 覆盖，默认 ~\.config\opencode\antigravity-notify.off
    - PushLog：OPENCODE_NOTIFY_LOG_FILE 覆盖，默认 %TEMP%\opencode\notify-push.log
    - TempDir / PluginFile / WidgetErrorLog / WidgetAliveFile：默认路径
    - AppConfigDir / AppConfigFile：linkWeixin 用户配置目录与配置文件
  #>
  $userProfile = $env:USERPROFILE
  $configDir = Join-Path $userProfile '.config\opencode'
  $tempDir = Join-Path $env:TEMP 'opencode'
  $appConfigDir = Join-Path $userProfile '.config\linkweixin'
  return @{
    ConfigDir         = $configDir
    TempDir           = $tempDir
    ...
    AntigravityMarker = if ($env:ANTIGRAVITY_NOTIFY_MARKER_FILE) { $env:ANTIGRAVITY_NOTIFY_MARKER_FILE } else { Join-Path $configDir 'antigravity-notify.off' }
    ...
    AppConfigFile    = Join-Path $appConfigDir 'config.json'
  }
}
