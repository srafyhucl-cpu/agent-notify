...
**恢复实测（2026-09-21 17:15–18:10，同一微信账号未更换）**：用户在**同一个微信号**上仅重新扫码绑定了一次，风控即在 48 小时内自行恢复：

| 证据 | 值 |
| --- | --- |
| 探针通知微信端可见 | 用户确认可见（`2026-09-21T09:15:51.844Z`，通知 `9caf6706-59a7-4e76-ba11-dc6b9f913e72`） |
| 被拒引用提示微信端可见 | 用户截图确认（`无法续聊：无可用会话记录…`，`2026-09-21T09:14:06Z`） |
| 真实任务通知批量可见 | 09:16–10:10 共 8 条 `【opencode】技术栈体积与小而美取舍 ponder`，用户确认**全部可见** |
| 平台受理统计 | 当前账号 `Sent 9` / `Skipped 3`，**无 `Failed`、无 `Unknown`** |

因此下文「为什么必须换第二个微信账号」的第 2 条理由（当前账号拿不到证据）**已不再成立**。

**处置规则**：

- 风控期间不再向该账号重复发送验收探针，避免加重限制；该限制已于 2026-09-21 17:15 自行解除（见上）。风控可能复发，日常仍应避免短时间大量重复发送。
...
2. **当前账号拿不到证据**：风控跟随微信用户账号（不跟随 Bot、代码或机器），`sendmessage` 返回 `ret=0` 与稳定 `message_id`、SQLite 记 `Sent`，但微信端什么都不显示。看不到通知就无法引用它，Step 3（用户可见）和 Step 4（引用→路由→Agent）都拿不到真实证据。
   **该理由已于 2026-09-21 17:15 失效**：风控自行恢复，当前账号已能取得可见证据（见「P0 根因」下的恢复实测）。
...
当前账号的状态（2026-09-21 17:15 之后）：上行与下行都已恢复，Step 3（微信可见推送）与 Step 4（引用→路由→Agent→Claim，含对照断言）均已取得真实证据，见「验收结果总览」与「回复腿首次真实闭环」两节。**第二账号现在只承担两件事**：Step 2 的独立账号隔离要求，以及 Step 5 的破坏性测试（`ret=-14` 会清 context token 并要求重新扫码，不应在唯一的生产账号上制造）。
...
**真实库的路由窗口（2026-09-21 16:40 核查，更正上文）**：上面“17:28–20:22 之前有效”说的是**虚拟化库**在 13:40 的 267 条路由。真实库首次启动后的实际状态是：

| 项目 | 真实库实测 |
| --- | --- |
| 路由总数 | 250，**全部属于 `clawbot-旧账号`**（当前账号） |
| `created_at` | 2026-09-15T03:35Z ～ 2026-09-19T15:47Z（Go 时代写入） |
| `expires_at` | 2026-10-15 ～ 2026-10-19（30 天 TTL，由 legacy 导入保留） |
| 未过期数量 | **250**（截至核查时全部有效） |

因此「回复腿」验证**不存在今天傍晚的截止时间**；真正的前置条件是下面这条。

**当前阻塞：账号会话已失效**。`%TEMP%\agent-notify\clawbot-debug.log` 最后一条记录为 `2026-09-21T10:55:48+08:00 getupdates {"errcode":-14,"errmsg":"session timeout"}`；真实库首次启动时已按设计把账号标记 `stale_at`、清空 `cursor_json` 并停止轮询。**在重新扫码登录之前，入站长轮询不会工作，微信里的引用回复收不到**，Step 4 也就无从验证。

恢复方式：在桌面端的 Channels 页用同一微信账号重新扫码登录（等价于产品的 `login` 流程），随后从该账号给 ClawBot 发一条消息重建 context。重新登录后的账号 ID 若与 `clawbot-旧账号` 不同，上面 250 条路由将不再匹配，需要重新确认路由归属。

**实测结果（2026-09-21 17:05–17:10，重新扫码后）**：账号 ID 确实变了，上一条警告成立。

| 观测 | 值 |
| --- | --- |
| 新账号 | `clawbot-当前账号`，`enabled=1` |
| 旧账号 | `clawbot-旧账号`，已被停用（`enabled=0`）并在 17:05 标记 `stale_at` |
| 新账号入站长轮询 | **已跑通**：`updated_at` 持续前进（09:09:25 → 09:10:02Z），`cursor_json` 118 字节 |
| 新账号路由数 | **0**；250 条路由仍全部属于已停用的旧账号 |
| 上下文状态 | `session_established_at` 仍为 null，需在新账号下重新发一条消息建立 context |

**结论：引用旧通知验证「回复腿」的路子彻底不成立** —— 入站消息会带新账号 ID，而路由按 `(channel_id, account_id, external_message_id)` 精确匹配，新账号一条路由都没有，必然 `NoExactRoute`。Step 3 / Step 4 只能走「新账号发新通知 → 引用这条新通知」。

**同时确认没有产生平台外发流量**：暂停被解除期间（17:03–17:06）有 4 次投递尝试，全部是 `Skipped/session_missing`（旧账号已失效、新账号上下文未建立），**没有一条 `Sent`**。

## 回复腿首次真实闭环（2026-09-21 17:44）

这是「引用 → 路由 → Agent → Claim」在生产账号上的首次完整闭环，也是 `sessionID` 契约修复（`d6eabb1`）之后出现的第一个成功样本。

| 时间(UTC) | 环节 | 证据 |
| --- | --- | --- |
| 09:44:01.684 | 微信引用回复被 ClawBot 收到 | `inbound_claims` 消息 `<平台消息ID已脱敏>`，账号 `clawbot-当前账号` |
| 09:44:02 | 精确命中路由（未命中会以 `Rejected(NoExactRoute)` 结束并回发可见提示） | ReplyRoute → `ses_f3e1d6d04ffeDQbHR42X6Qz08O` |
| 09:44:02 | 插件投递成功 | `opencode-reply-inbox/results/79f099ca-….json` = `{"ok":true,"error":""}` |
| 09:44:03.052 | Claim 终态 | `InProgress` → **`Completed`** |
| 09:44:03.374 | 正文落到目标会话 | `session_message` seq 2767，`type=user`，正文为微信中实际输入的文本 |
| 对照断言 | 同一时间窗（09:43:50–09:44:20）内**全库只有这一个会话**产生消息 | 未串入任何其他会话 |
...
1. **未使用独立测试账号（已豁免，风险自留）**：当前使用用户生产 ClawBot 账号。计划原文要求"必须使用独立测试账号，否则不能记为隔离验收通过"，该条已由计划负责人于 2026-09-21 **明确豁免**，改用「生产账号 + 隔离目录」（数据隔离、账号不隔离）。**代价与残余风险**：(a) 风控一旦复发，推送与引用回复会同时失去用户可见性，而独立账号本可隔离这一风险；(b) 破坏性测试（强制 `ret=-14` 重新扫码、`ret=-2`）不得在唯一生产账号上制造，Step 5 改用「隔离实例 + 故意损坏的 context token」触发真实平台响应；(c) 所有 `Sent` 证据均来自生产账号，跨账号作用域未覆盖。
2. ~~**微信端展示未确认**~~（**已解除**，2026-09-21 17:15）：用户已在微信端确认收到探针通知与被拒引用提示；09:16–10:10 的 8 条真实任务通知亦确认全部可见。
3. ~~**引用回复未闭环**~~（**已解除**，2026-09-21 17:44）：Claim 已从 `InProgress` 变为 `Completed`，正文落到目标会话，对照断言以全库检索完成。剩余：第 5 项「重复投递不重复执行」未在微信端复测（仅有单测证据）。
