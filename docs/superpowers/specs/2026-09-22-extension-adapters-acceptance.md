...
1. ~~**2.0.3 发布**~~：✅ 已完成，见第 12 节。
2. ~~**验收回归**~~：✅ 已完成（推送 + 引用回复均通过），见第 12 节。

## 12. 2.0.3 发布记录与自愈真机验证（2026-09-23）

### 发布

| 项 | 值 |
| --- | --- |
| tag | `v2.0.3`（指向 `8b55aaf`） |
| 修复提交 | `c133400`（自愈重试）、`2f2b36e`（重试前先判定状态，避免正常启动白白重启一次运行时） |
| Release workflow | 运行 `35816803765`，**全部步骤成功**（含首次启用草稿式镜像） |
| 产物 | `Agent-notify-Setup-v2.0.3.exe`（8,031,616 B / SHA256 `33e39779…a3f8`）、`Agent-notify-v2.0.3.zip`（9,117,972 B / `18c4e270…ac56`）、`SHA256SUMS.txt` |
| 镜像 | 二进制仓库 `srafyhucl-cpu/agent-notify-releases`：`/releases/latest` = `v2.0.3`，**无草稿残留** ✅（新的"草稿创建 → 上传全部资产 → `--draft=false --latest`"流程生效） |
| 独立验证 | SHA256 与 `SHA256SUMS.txt` 一致 ✅；安装器 + ZIP 内五个程序签名指纹均为 `EDF9E283…09ACD` ✅；安装器 `ProductVersion=2.0.3.0`、ZIP 内 `VERSION=2.0.3` ✅ |

### 自愈真机验证（心跳注入 + 真实安装器）

Rust → Rust 升级不会自然触发"旧版心跳仍新鲜"（`widget-alive.txt` 早已过期），因此升级前向 `%TEMP%\agent-notify\widget-alive.txt` 注入一份当前时间的心跳，再用与一键升级**完全相同**的参数（`/SILENT /NORESTART /LOG=`）静默安装 2.0.3：

| 时间 | 日志/事件 | 含义 |
| --- | --- | --- |
| 13:14:07 | 注入心跳 | 模拟旧版刚被安装器杀掉 |
| 13:14:22 | `host_version=2.0.3 … 启动桌面运行时` | 新版首启 |
| 13:14:24 | `WARN 旧数据迁移失败，启动迁移诊断模式 code=legacy_app_running` | 故障复现（与 2.0.2 时的现象一致） |
| 13:14:59 | `旧版心跳失效后迁移自愈完成，运行时已恢复正常 attempts=1` | **35 秒后首次重试即自愈** |
| 13:15:17 | `ClawBot notifystart 已发送，主动推送会话就绪` | 渠道恢复 |
| — | 用户确认：收到推送，且引用回复进入正确会话 | 推送 + 精确回复全链路通过 |

**结论**：升级后首启落入诊断模式时**无需任何人工操作**即可恢复（此前必须手动重启应用或在界面点"重新检测"）；恢复后无多余重启（重试循环按设计在恢复后停止）。

**普通重启的无多余重启核对（已完成）**：托盘退出后重新打开（13:26:34 停止 → 13:26:39 启动，`host_version=2.0.3`），启动即通过迁移检查（**无**诊断模式）；此后日志中**没有**任何新的 `停止/启动桌面运行时`，`迁移自愈完成` 全文仅 1 次（即上面的注入测试）→ 自愈重试的前置判定生效，正常启动不会被多余重启。同一次启动还排空了 spool 里残留的 2 个文件并完成去重：通知总数 575 → 579，与新增事件数一一对应，**无重复投递**。

## 13. Go 1.17 → 2.0.3「自动检查更新」真机验收（2026-09-23）

为验证缺陷 2（镜像半成品窗口）修复后的**自动检查**路径，把机器回退到 Go 1.17.0（卸载 2.0.3 → 静默安装 1.17.0 → 用 Go 版 `install.ps1 -ConfigureOnly` 还原插件/Hook/mod，OpenCode 插件烘焙真实路径），然后**只等自动检查、不点「检查更新」**：

| 时间 | 事件 | 结论 |
| --- | --- | --- |
| 13:30:57 | Go 版启动后 15 秒的静默检查发现 `v2.0.3`，按钮标黄 | **自动检查路径生效**（正是原先选错路径的那条） |
| 13:50 | 首次点升级 → `下载校验文件失败：HTTP 403 API rate limit exceeded` | 环境问题：Go 更新器走匿名 API（60 次/小时，同 IP 共享），已写入排障文档 |
| 14:31:54 | 配额重置后重试 → 下载 **`Agent-notify-Setup-v2.0.3.exe`**（8,031,616 B） | **安装器分支** ✅，不再回退到 ZIP |
| 14:32:11 | 静默安装完成、自动重启（`agentnotify-desktop`） | 一键升级体验 ✅ |
| 14:32:14 | `旧数据迁移失败，启动迁移诊断模式 code=legacy_app_running` | Go 版心跳新鲜的必然结果 |
| 14:32:49 | `旧版心跳失效后迁移自愈完成，运行时已恢复正常 attempts=1` | **35 秒后首次重试即自愈**，零人工介入 ✅ |
| 14:33:07 | `ClawBot notifystart 已发送，主动推送会话就绪` | 渠道恢复 ✅ |
...
**真机验证（2026-09-23 17:01）**：安装 2.0.5 后页脚显示 `09/23 17:01`（本地时间），对应投递 `state=Sent`（09:01:10Z）✅；同一位置在修复前显示 `06:34`（UTC）。该修复包含在 2.0.4 起的所有版本中，本次在 2.0.5 上完成真机验收。

## 15. 2.0.5 发布记录：应用内升级误拒 32 位安装器（2026-09-23）

### 缺陷

- **现象**：2.0.3 点「下载并安装」报「更新包不是 64 位 Windows 可执行文件」。
- **根因**：更新器要求更新包必须是 64 位 PE（`PE32+`），而 Inno Setup 的安装器存根是 **32 位 PE**（实测 `Machine=0x014C`）→ 误拒。旧版（Go 1.17）之所以正常：它的安装器分支只校验 SHA256、文件头 `MZ` 与签名指纹，**从不检查位宽**。
- **影响面**：2.0.0–2.0.4 的应用内升级不可用（2.0.3 → 2.0.4 是它的首次实战，被真机拦下）。
- **修复**（提交 `ed3a4a1`）：引入 `PeBitness` —— 安装器通道 `Any`（保留 SHA256、文件版本、签名三道校验），替换进安装目录的应用程序本体 `Require64`；回归测试用真实 32 位 PE（`SysWOW64\notepad.exe`）验证两条通道的判定。
- **升级路径**：2.0.0–2.0.4 的更新器会以同一原因拒绝 2.0.5 的安装器，这些版本**无法应用内升级**，需手动安装一次（已写入 CHANGELOG「说明」与 `docs/TROUBLESHOOTING.md`）。

### 发布与两次发布链问题

| 项 | 值 |
| --- | --- |
| tag | `v2.0.5`（修复 `ed3a4a1`、发布 `d61eed0`、文档 `224cf0a`） |
| 首次运行 | `35834953244`：构建、发布、镜像三步都报成功，但 `/releases/tags/v2.0.5` 的内嵌 `assets` 数组为空 → 一度误判「空发布」。用 `/releases/<id>/assets` 复核确认**三个资产都在**（`uploaded`）——是 tags 端点的缓存滞后；`gh release view/download` 依赖该端点，故同样看不到。重跑时 `gh release upload --clobber` 因 `HTTP 422 ReleaseAsset.name already exists` 失败，属同一缓存问题的连带效应。 |
| 门禁补强 | `35de425` 新增 **Verify release assets**：逐仓库校验非草稿 + 三个资产齐备，缺失即让整个发布失败；`3651136` 把校验数据源从 tags 端点改为独立的 assets 端点，避免缓存滞后造成误报。 |
| 产物 | `Agent-notify-Setup-v2.0.5.exe`（8,036,184 B / SHA256 `e9b64aa1…`）、`Agent-notify-v2.0.5.zip`（9,123,247 B）、`SHA256SUMS.txt` |
| 独立验证 | 直链下载后 SHA256 与 `SHA256SUMS.txt` 一致 ✅；安装器签名指纹 `EDF9E283…09ACD`、`ProductVersion=2.0.5.0` ✅；两仓库 `/releases/<id>/assets` 均为 3 个 `uploaded`，`/releases/latest` = `v2.0.5` ✅ |
| 安装方式 | 本次为手动静默安装（`/SILENT /NORESTART /LOG=`，与一键升级同参数）：安装成功、无 AgentNotify 相关挂起替换项、`agentnotify-desktop.exe` 文件版本 2.0.5 ✅（安装过程中出现过一次「无法自动关闭所有应用」提示，确认当时已无残留进程占用，属过期列表，重试后正常） |

### 真机验证（2026-09-23 17:01）

- 安装后投递记录出现 `state=Sent`（09:01:10Z），用户确认微信页脚显示 `09/23 17:01`（本地时间）→ **2.0.4 的页脚修复在 2.0.5 上完成真机验收** ✅
- 期间发现 ClawBot 主动推送会话失效（`session_missing`，16:42 起）：给 bot 发一条消息后 16:59:04 重建成功 ✅（预期机制，非缺陷）
- **遗留待办**：应用内升级的端到端验证要等 2.0.5 → 下一个版本（这条路径的首次实战）；2.0.0–2.0.4 用户需手动安装一次 2.0.5。
