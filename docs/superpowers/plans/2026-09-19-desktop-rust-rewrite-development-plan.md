...
方案包含多个可独立验收的子系统，因此开发计划拆为一份总计划和四份执行计划：
...
执行顺序固定为 `Rust 核心 -> Tauri/UI -> 生产闭环与切换 -> 扩展生态`。后一份计划不得绕过前一阶段门禁提前进入主线；可以使用假适配器并行做 UI 设计，但不得用假实现替代真实验收。
