...
- 「测试发送」结果改为**浮层提示**：固定出现在窗口底部中央、3 秒后自动消失，不再占用页面顶部空间；文案固定为「已发送测试消息。」，失败仍用可读的行内错误并保持可见。

### Fixed

- **升级后界面停留在旧版**：WebView2 的磁盘缓存按 URL 保留着上一版的 `index.html` 与前端包，替换主程序后仍会加载旧页面。安装器现在会在停止旧进程、替换文件之前清理 `Cache` / `Code Cache` / `GPUCache` 三类缓存（保留 Local Storage，自定义账号名与主题选择不受影响），手动覆盖安装与应用内升级都会执行。
- 发布构建新增守卫：断言主程序确实嵌入了当前前端构建的全部资源名，避免「前端重建但宿主没有重新编译」的陈旧嵌入被发布出去。
- 发布链路：源码仓 Release 草稿改为按列表匹配并按 release id 发布（`GET /releases/tags/{tag}` 对草稿返回 404，旧写法会误判「草稿消失」而中止发布）；可复用发布 workflow 补声明 `RELEASE_REPO_TOKEN`、构建任务补齐仓库根 `node_modules`；补发脚本新增 `-AllowPublished` 显式覆盖开关。
...
- 渠道页「测试发送」收进账号行：每个账号在「启用」后面直接一个按钮，点一下即发送内置内容（标题固定、正文带该账号名），不再需要手选账号与手写正文；右侧面板下线，页面改为单列。
...
- 引用回复的结果文件读取容忍 Windows 上杀软/索引器的瞬时占用（共享冲突）：按 10×20ms 有界重试后才报错，避免偶发的「读取结果失败」假失败；结果文件写入侧保持 temp + rename 原子写。
