...
右下角无边框小窗（`linkweixin-widget.ps1`）：

- **双开关**：opencode / codex 各一个按钮（绿 ON / 红 OFF），顶条与托盘图标同步
- **运行灯**：两个 agent 进程是否在跑；**上次推送**：读推送日志尾行；底栏显示版本号
- 拖标题区移动；右上角 × / — 藏到托盘；双击托盘图标恢复
- 右键托盘菜单可开关推送或彻底退出；右下角红字"退出"也可关闭
- 重启：桌面双击「linkWeixin 悬浮窗」（单实例自动接管）；开机自启靠 `shell:startup`
